    ...
    for i, p in enumerate(papers, 1):
        try:
            ...
            if getattr(p, "authors", None):
                lines.append(
                    f"  author = {{{' and '.join(_bibtex_escape(a) for a in p.authors)}}},"
                )
            if getattr(p, "journal", None):
            ...
            if getattr(p, "keywords", None):
                lines.append(
                    f"  keywords = {{{', '.join(_bibtex_escape(kw) for kw in p.keywords)}}},"
                )
            if getattr(p, "oa_url", None):
                lines.append(f"  url = {{{_bibtex_escape(p.oa_url)}}},")
            elif getattr(p, "doi", None):
                lines.append(
                    f"  url = {{{_bibtex_escape(f'https://doi.org/{p.doi}')}}},"
                )
            # Phase 5a: 学术元数据
            if getattr(p, "orcid", None):
...
def export_endnote_xml(papers: list) -> str:
    ...
    for p in papers:
        ...
        if p.authors:
            ...
            for author in p.authors:
                ...
                if "," in author:
                    ...
                    lines.append(
                        f'<author><style face="normal" font="default" size="100">{_xml_escape(parts[0].strip())}, {_xml_escape(parts[1].strip())}</style></author>'
                    )
                else:
                    lines.append(
                        f'<author><style face="normal" font="default" size="100">{_xml_escape(author)}</style></author>'
                    )
            lines.append("</authors>")
        lines.append("</contributors>")
        ...
        titles_children = []
        if p.title:
            titles_children.append(
                f'<title><style face="normal" font="default" size="100">{_xml_escape(p.title)}</style></title>'
            )
        if getattr(p, "conference", None):
            titles_children.append(
                f'<secondary-title><style face="normal" font="default" size="100">{_xml_escape(p.conference)}</style></secondary-title>'
            )
        if titles_children:
            lines.append("<titles>")
        ...
        if p.journal:
            lines.append(
                f'<periodical><full-title><style face="normal" font="default" size="100">{_xml_escape(p.journal)}</style></full-title></periodical>'
            )
        if p.year:
            lines.append(
                f'<dates><year><style face="normal" font="default" size="100">{p.year}</style></year></dates>'
            )
        # 补充学术字段
        if getattr(p, "volume", None):
            lines.append(
                f'<volume><style face="normal" font="default" size="100">{_xml_escape(p.volume)}</style></volume>'
            )
        if getattr(p, "issue", None):
            lines.append(
                f'<number><style face="normal" font="default" size="100">{_xml_escape(p.issue)}</style></number>'
            )
        if getattr(p, "pages", None):
            lines.append(
                f'<pages><style face="normal" font="default" size="100">{_xml_escape(p.pages)}</style></pages>'
            )
        if getattr(p, "issn", None):
            lines.append(
                f'<isbn-issn><style face="normal" font="default" size="100">{_xml_escape(p.issn)}</style></isbn-issn>'
            )
        if p.doi:
            lines.append(
                f'<electronic-resource-num><style face="normal" font="default" size="100">{_xml_escape(p.doi)}</style></electronic-resource-num>'
            )
        if p.abstract:
            abs_text = _xml_escape(p.abstract[:2000])
            lines.append(
                f'<abstract><style face="normal" font="default" size="100">{abs_text}</style></abstract>'
            )
        if p.keywords:
            lines.append("<keywords>")
            for kw in p.keywords:
                lines.append(
                    f'<keyword><style face="normal" font="default" size="100">{_xml_escape(kw)}</style></keyword>'
                )
            lines.append("</keywords>")
        # Phase 5a: 学术元数据
        ...
        if getattr(p, "orcid", None):
            ...
            lines.append(
                f'<custom1><style face="normal" font="default" size="100">{orcid_value}</style></custom1>'
            )
        # conference is already included in <titles> block above
        if getattr(p, "funding", None):
            for fund in p.funding:
                lines.append(
                    f'<custom3><style face="normal" font="default" size="100">{_xml_escape(fund)}</style></custom3>'
                )
        if p.pmid:
            lines.append(
                f'<accession-num><style face="normal" font="default" size="100">{_xml_escape(p.pmid)}</style></accession-num>'
            )
        url = p.oa_url or (f"https://doi.org/{p.doi}" if p.doi else "")
        if url:
            lines.append(
                f'<urls><related-urls><url><style face="normal" font="default" size="100">{_xml_escape(url)}</style></url></related-urls></urls>'
            )
        lines.append("</record>")
