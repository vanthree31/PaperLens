...
| **多数据源聚合** | PubMed + OpenAlex（+ Google Scholar / Bing 学术 / CNKI / 万方 / 维普 实验性），自动去重、补充引用数和 OA 链接 |
| **批量 DOI 导入** | 粘贴多个 DOI，一键查询 |
| **高级筛选** | 按期刊、字段标签、年份范围、文献类型过滤 |
| **结果内排序** | 按被引数、时间、标题、近期影响力（引用增长速率）即时排序 |
...
| **关键词共现网络** | 可视化搜索结果中的关键词共现关系 |
| **作者合作网络** | 可视化搜索结果中的作者合作关系 |

### 导出与集成

| 功能 | 说明 |
|------|------|
| **多格式导出** | RIS（EndNote）、BibTeX（LaTeX）、CSV、EndNote XML — 含学术字段（卷号、期号、页码、ISSN） |
| **PDF 下载** | 直接下载 OA 论文 PDF，带链接验证 |
| **搜索历史** | 持久化保存，支持智能建议 |
| **双 AI 模型** | 检索用快速模型，分析用高质量模型，独立配置 max_tokens |
| **Zotero 同步** | 直接同步论文到 Zotero，支持选择收藏夹 |
| **阅读推荐** | 个性化推荐：基于关键词的深耕性推荐 + 基于引用关系的探索性推荐 |
...
### 引用图谱探索
点击任意论文的 **引用关系图谱** 按钮 → 在交互式图谱中查看引用关系 → 点击节点展开引用。
...
- [x] 多数据源聚合（PubMed + OpenAlex + Google Scholar / CNKI / 万方 / 维普）
- [x] AI 论文分析（总结 / 详析 / 对比 / 创新点）
- [x] 引用关系图谱（D3.js 交互式可视化）
- [x] 悬浮图谱窗口（可拖动，完整 AI 工具栏）
- [x] 创新点检测（AI 识别研究空白）
- [x] 批量 DOI 导入
- [x] 多格式导出（RIS / BibTeX / CSV / EndNote XML）含学术字段
- [x] 双 AI 模型配置（独立 max_tokens）
- [x] AI 分析流式输出
