...
from core.utils import _check_url_safety
...
def export():
    ...
    if isinstance(direct_papers, list) and len(direct_papers) > 0:
        ...
        selected = (
            [papers[i] for i in indices if 0 <= i < len(papers)] if indices else papers
        )
    ...
    _title = (
        ...
        else getattr(_first, "title", None)
    )
    if _title:
        first_title = re.sub(r"[^\w\-]", "_", _title[:50]).strip("_")
    safe_name = (
        first_title or re.sub(r"[^\w\-]", "_", query[:40]).strip("_") or "papers"
    )
    ...
    if fmt == "ris":
        content, filename, mime = (
            export_ris(selected),
            f"{safe_name}_{timestamp}.ris",
            "application/x-research-info-systems",
        )
    elif fmt == "bibtex":
        content, filename, mime = (
            export_bibtex(selected),
            f"{safe_name}_{timestamp}.bib",
        ...
        content, filename, mime = (
            export_endnote_xml(selected),
            f"{safe_name}_{timestamp}.xml",
            "application/xml",
        )
    ...
    if save_to_disk:
        export_dir = load_config().get("export_path", "") or os.path.join(
            os.path.expanduser("~"), "PaperLens_Exports"
        )
        try:
            ...
            sub = sub_map.get(
                fmt,
                "Citations"
                if fmt in ("apa", "mla", "gb7714", "chicago", "vancouver")
                else "Other",
            )
    ...
    try:
        ...
        while resp.status_code in (301, 302, 303, 307, 308) and redirect_count < 5:
            ...
            if not safe:
                ...
                return jsonify(
                    {"error": err}
                ), 403 if err == "blocked_internal_url" else 400
        ...
        if save_to_disk:
            ...
            try:
                ...
                if not (
                    real_filepath.startswith(real_export + os.sep)
                    or real_filepath == real_export
                ):
                    return jsonify({"error": "invalid_path"}), 400
                # 流式写入，限制大小
                total_size = 0
